...
export class WsLimitedPriceProvider implements IPriceProvider {
    ...
    async getPrice(): Promise<number> {
        ...
        if(this._exchanges.length > 0) {
            for (let exchange of this._exchanges) {
                let priceInfo = this._ex2priceInfo[exchange.ex];
                let conversionRate:number = await this.getUsdtConversionRate( exchange.market );

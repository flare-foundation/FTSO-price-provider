...
Whitelisting is a fully decentralized process of selection of data providers with the most vote power. This data provider implementation takes care of whitelisting if configured so.

**IMPORTANT**: The term *whitelisting* as used in this document is not to be confused with IP whitelisting of validators nodes to be able to connect to core network or whitelisting of IP address of a server to access a private API nodes or any other IP based whitelisting.

There are two types of data providers, ordinary ones and trusted data providers. Trusted data providers are decided by a governance process and do not need to become whitelisted, as they are whitelisted by default. During usual operations of FTSO system, trusted data providers have exactly the same role as ordinary data providers. Only in exceptional situations when FTSO system goes into a special fallback mode these data providers are relied on to provide prices from which average is calculated. Special fallback mode kicks if too small number of data providers casts their votes (too small in terms of vote power) or if price changes abruptly or if it is triggered by governance. In special fallback mode no rewards are distributed to trusted data providers.
...
- `list of tuples`: each tuple consists of two values. First is the external exchange name (eg. `bitstamp`, `kraken`, `binanceus`), while the second is the pair name on that exchange (eg. `XRPUSD`, `xrpusd`, `XRP-USD`). The list may be arbitrarily long and serves as a list of priorities, that is first we try to retrieve the price from the first tuple in the list, then from the second, etc. Then depending on the last parameter - `mode` - we calculate the price and feed it to *Songbird Network*.
